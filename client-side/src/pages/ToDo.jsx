...
const ToDo = () => {
  ...
  const saveEdit = async () => {
    const updatedTask = await updateTodo(editingTask._id, {
      title: editTitle,
      note: editNote,
      icon: editIconName,
    });
    ...
    setTasks((prev) =>
      prev.map((item) =>
        item._id === editingTask._id ? { ...item, ...updatedTask } : item,
      ),
    );
  ...
  const toggleComplete = async (id) => {
    try {
      ...
      setTasks((prev) =>
        prev.map((t) => {
          if (!t.completed && t._id === id) setFireSound(true);
          return t._id === id ? { ...t, ...updated } : t;
        }),
      );
    } catch (error) {
  ...
  useEffect(() => {
    if (fireSound) {
      const sound = new Audio("/sounds/achievementSound.mp3");
      setFireSound(false); // reset if sound fails to play
  ...
  const [fireDeleteSound, setFireDeleteSound] = useState(false);

  const deleteTask = async (id) => {
    await deleteTodo(id);
    setTasks(
      tasks.filter((t) => {
        setFireDeleteSound(true);
        return t._id !== id;
      }),
    );
  };
  ...
  const filteredTasks = tasks.filter(
    (t) => normalizeDate(t.date) === selectedDate && !t.completed,
  );
            ...
            {showCalendar && (
              <div className="absolute top-16 sm:top-14 right-0 w-full sm:w-72 bg-white border border-slate-200 rounded-2xl shadow-2xl p-4 animate-in fade-in zoom-in duration-200">
                <div className="flex justify-between items-center mb-4 text-sm font-bold">
                  <button className="p-2 hover:bg-slate-100 rounded-lg icon-btn-press">
                    <ChevronLeft size={18} />
                  </button>
              ...
              <div className="space-y-4">
                {filteredTasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task._id}
                      className="group bg-white p-5 rounded-xl border border-slate-300 shadow-sm flex items-center gap-4 sm:gap-5 hover:border-purple-200 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 relative card-radius"
                    >
                      <button
              ...
              <div className="space-y-4">
                {completedTasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task._id}
                      className="bg-slate-100/60 p-5 rounded-xl border border-dashed border-slate-300 flex items-center gap-4 sm:gap-5 opacity-60 grayscale hover:grayscale-0 transition-all card-radius"
                    >
                      <button
                        onClick={() => toggleComplete(task._id)}
                      ...
                      <div className="flex-1 flex items-center gap-3 min-w-0">
                        <IconComp
                          size={18}
                          className="text-slate-400 icon-hover-effect shrink-0"
                        />
          ...
          <div className="edit-toast-overlay fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="edit-toast-container w-full max-w-sm bg-white border border-purple-200 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.18)] p-6 rounded-2xl pointer-events-auto animate-toast-pop">
              <div className="flex justify-between items-center mb-5">
                <h4 className="text-xs font-black uppercase tracking-[0.2em] text-purple-600">
                  Edit Task
                </h4>
                <button
